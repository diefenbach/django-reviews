...
def save(request):
    ...
    if form.is_valid():
        ...
        # Fire up signal
        reviews_signals.review_added.send(new_review)

        # Save data of the rated object within session (for thank-you page)
        request.session["last-rated-object"] = [new_review.content_type_id, new_review.content_id]
...
def thank_you(request, template_name="reviews/thank_you.html"):
    ...
    if "last-rated-object" in request.session:
        ...
        obj = obj_type.get_object_for_this_type(pk=content_id)
