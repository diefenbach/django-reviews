...
def get_best_rated_for_model(instance):
    ...
    result = (
        Review.objects.filter(content_type=ctype.id, active=True)
        .values("content_id")
        .annotate(Avg("score"))
        .order_by("-score__avg")
        .first()
...
def has_rated(request, instance):
    ...
    try:
        if request.user.is_authenticated:
            Review.objects.get(content_type=ctype.id, content_id=instance.id, user=request.user)
        else:
            Review.objects.get(content_type=ctype.id, content_id=instance.id, session_id=request.session.session_key)
